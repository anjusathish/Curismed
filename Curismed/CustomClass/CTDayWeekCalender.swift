//
//  CTDayWeekCalender.swift
//  Kinder Care
//
//  Created by CIPL on 12/4/19.
...
class CTDayWeekCalender: UIControl {
  ...
  var dateString : String{
    get {
      let dateString = date.asString(withFormat: "yyyy-MM-dd")
      return dateString
    }
  }
  ...
  func resetDayView(){
    ...
    if tag > 1 {
      if currentDate != "\(tag)" ,selectedView!.subviews.count > 2 {
    ...
    if tag < maxDaysCount {
      let nextView = superViewDay?.subviews[tag]
      if currentDate != "\(tag)",(nextView?.subviews.count)! > 2 {
